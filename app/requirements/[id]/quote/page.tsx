    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg w-full border border-red-100">
          <h2 className="text-red-600 font-bold text-xl mb-4">Production Error</h2>
          <pre className="bg-red-50 p-4 rounded-lg text-[10px] overflow-auto text-red-800">
            {error.message}
            {"\n\n"}
            {error.stack}
          </pre>
          <p className="mt-4 text-sm text-muted">
            Check if the database is synchronized: run <code className="bg-sand px-1">npx prisma db push</code>
          </p>
        </div>
      </div>
    );
